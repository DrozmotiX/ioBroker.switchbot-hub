...
class SwitchbotHub extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (!this.config.openToken) {
      this.log.error(
        "*** No token provided, Please enter your token in adapter settings !!!  ***",
      );
    }

    // Load intervall settings
    intervallSettings.all =
      this.config.intervallAll != null
        ? this.config.intervallAll * 60000 || intervallSettings.all
        : intervallSettings.all;
    intervallSettings.Curtain =
      this.config.intervallCurtain != null
        ? this.config.intervallCurtain * 60000 || intervallSettings.Curtain
        : intervallSettings.Curtain;
    intervallSettings.Humidifier =
      this.config.intervallHumidifier != null
        ? this.config.intervallHumidifier * 60000 ||
          intervallSettings.Humidifier
        : intervallSettings.Humidifier;
    intervallSettings.Meter =
      this.config.intervallMeter != null
        ? this.config.intervallMeter * 60000 || intervallSettings.Meter
        : intervallSettings.Meter;
    intervallSettings.Plug =
      this.config.intervallPlug != null
        ? this.config.intervallPlug * 60000 || intervallSettings.Plug
        : intervallSettings.Plug;
    intervallSettings.SmartFan =
      this.config.intervallSmartFan != null
        ? this.config.intervallSmartFan * 60000 || intervallSettings.SmartFan
        : intervallSettings.SmartFan;
  ...
  defineIntervallTime(deviceId) {
    try {
      ...
      if (!this.devices[deviceId] || !this.devices[deviceId].deviceType) {
        return;
      }
  ...
  apiCall(url, data) {
    ...
    if (!url) {
      throw new Error(`No URL provided, cannot make API call`);
    }
    if (!data) {
  ...
  async loadDevices() {
    try {
      ...
      this.log.debug(
        `[getDevices API response]: ${JSON.stringify(apiResponse)}`,
      );
      if (!apiResponse) {
      ...
      const arrayHandler = async (deviceArray) => {
        for (const device in deviceArray) {
          ...
          await this.extendObjectAsync(
            `${deviceArray[device].deviceId}._info`,
            {
              type: "channel",
              common: {
          ...
          for (const infoState in deviceArray[device]) {
            await this.stateSetCreate(
              `${deviceArray[device].deviceId}._info.${infoState}`,
              infoState,
              deviceArray[device][infoState],
            );
          }
          ...
          switch (deviceArray[device].deviceType) {
            case "Bot":
              await this.stateSetCreate(
                `${deviceArray[device].deviceId}.press`,
                `press`,
                null,
              );
              await this.stateSetCreate(
                `${deviceArray[device].deviceId}.state`,
                `ON/OFF`,
                null,
              );
              break;
          ...
          this.log.debug(
            `[deviceStatus for ]: ${JSON.stringify(this.devices[deviceArray[device].deviceId].deviceName)}`,
          );
          await this.deviceStatus(deviceArray[device].deviceId);

          // Define intervall time (only if device has states)
          if (
            this.devices[deviceArray[device].deviceId] &&
            this.devices[deviceArray[device].deviceId].states
          ) {
            try {
      ...
      this.log.info(
        `Connected to SwitchBot API found ${deviceList.length} devices`,
      );
      ...
      try {
        ...
        if (infraredRemoteList != null) {
          ...
          this.log.error(
            `Can not handle infrared remote list from SwitchBot API`,
          );
        }
      } catch (error) {
      ...
      this.log.debug(
        `All devices configuration data : ${JSON.stringify(this.devices)}`,
      );
    } catch {
  ...
  async deviceStatus(deviceId) {
    try {
      ...
      this.log.debug(
        `[deviceStatus apiResponse ]: ${JSON.stringify(apiResponse)}`,
      );
      if (!devicesValues || Object.keys(devicesValues).length === 0) {
        this.log.debug(
          `No States found for type ${this.devices[deviceId].deviceType}`,
        );
        return;
      }
      ...
      for (const statusState in devicesValues) {
        await this.stateSetCreate(
          `${deviceId}.${statusState}`,
          statusState,
          devicesValues[statusState],
        );
  ...
  async infraredRemoteDevices(remoteArray) {
    try {
      for (const remoteControl in remoteArray) {
        this.devices[remoteArray[remoteControl].deviceId] =
          remoteArray[remoteControl];
        ...
        for (const infoState in remoteArray[remoteControl]) {
          await this.stateSetCreate(
            `${remoteArray[remoteControl].deviceId}._info.${infoState}`,
            infoState,
            remoteArray[remoteControl][infoState],
          );
        }
        ...
        if (!irDeviceButtons[remoteArray[remoteControl].remoteType]) {
          this.log.error(
            `IR Remote Type ${[remoteArray[remoteControl].remoteType]} not yet implemented`,
          );
        }

        const allIrButtons =
          irDeviceButtons[remoteArray[remoteControl].remoteType];
        ...
        for (const irButton in allIrButtons) {
          const common = {
            ...
            type:
              allIrButtons[irButton] !== undefined
                ? allIrButtons[irButton].type || "number"
                : "number",
            role:
              allIrButtons[irButton] !== undefined
                ? allIrButtons[irButton].type || "button"
                : "button",
            write: true,
          };
          ...
          await this.extendObjectAsync(
            `${remoteArray[remoteControl].deviceId}.${stateName}`,
            {
              type: "state",
              common,
            },
  ...
  async stateSetCreate(stateName, name, value) {
    this.log.debug(
      `Create_state called for : ${stateName} with value : ${value}`,
    );
  ...
  async onStateChange(id, state) {
    try {
      if (state && state.ack === false) {
        ...
        if (deviceType) {
          // State change regular device  detected
          switch (deviceType) {
            case "Bot":
          ...
          // State change of IR Remote  detected
          apiData.parameter = `default`;
        ...
        try {
          this.log.debug(
            `[sendState] ${JSON.stringify(this.devices[deviceId])}: ${JSON.stringify(apiData)}`,
          );
          const apiResponse = await this.apiCall(
            `${apiURL}`,
          ...
          this.log.debug(
            `[sendState apiResponse]: ${JSON.stringify(apiResponse)}`,
          );
  ...
  sendSentry(msg, error) {
    ...
    if (error) {
      sentryMessage = `${msg} | Error : ${error} | StackTrace : ${error.stack}}`;
    }
    ...
    if (!disableSentry) {
      if (this.supportsFeature && this.supportsFeature("PLUGINS")) {
        ...
        if (sentryInstance) {
          this.log.info(
            `[Error caught and sent to Sentry, thank you for collaborating!]  ${sentryMessage}`,
          );
